...
/*   Updated: 2017/11/27 17:41:59 by jgonthie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "JEAAAAAAAAAAAAN.h"

static _Bool	ini_ty_param(t_op *tab, t_order *slot, char *param)
{
	int		index;
	...
	while (++index < slot->nb_param)
	{
		...
		if (tab->tp_param[index] & T_REG)
			if (!reg_param(&slot->param[index], &slot->ty_param[index], param))
				return (0);
		if (tab->tp_param[index] & T_DIR)
			if (!dir_param(&slot->param[index], &slot->ty_param[index], param))
				return (0);
		if (tab->tp_param[index] & T_IND)
			if (!ind_param(&slot->param[index], &slot->ty_param[index], param))
				return (0);
...
_Bool			parse_params(char **tokens, int max, t_order *slot, ssize_t op_idx)
{
	...
	while (++index < REG_NUMBER)
	{
		if (g_op_tab[index].op_code == slot->op_code)
		{
			...
			if (!ini_ty_param(&g_op_tab[index], slot, tokens[op_idx + 1]))
			{
				index = -1;
				while (++index < slot->nb_param)
					if (slot->param[index])
