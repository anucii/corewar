...
static int  ft_count_words(char *s)
{
    ...
    index = 0;
    count = 0;
    while (s[index] && (s[index] == SPACE || s[index] == TAB || s[index] == COMA))
        index++;
    while (s[index])
    {
        if (s[index] && s[index] != SPACE && s[index] != TAB && s[index] != COMA)
        {
            while (s[index + 1] && s[index] != SPACE && s[index] != TAB && s[index] != COMA)
                index++;
            count++;
        }
        index++;
